...
class DownConvBlock(nn.Module):
    ...
    def __init__(
        ...
        if pool:
            self.layers.append(nn.MaxPool3d(kernel_size=2, stride=2, padding=0))
            # self.layers.append(nn.Conv3d(input_dim, input_dim, kernel_size=2, stride=2, padding=0))
...
class UpConvBlock(nn.Module):
    ...
    def forward(self, right, left):
        ...
        padding = (left_shape[4] - right_shape[4], 0, left_shape[3] - right_shape[3], 0, left_shape[2] - right_shape[2], 0)

        right_pad = nn.ConstantPad3d(padding, 0)
