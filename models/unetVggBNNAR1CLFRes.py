...
class metaCLF(nn.Module):
    ...
    def __init__(self, dis_cs, in_cs):
        ...
        self.res = nn.Conv3d(in_cs, 1,1,bias=False)    
    
    def forward(self, x, d_all):
        
        filters = self.generate_filters(d_all)
        x = self.res(x) + (x * filters).sum(dim=1, keepdim=True)

        return x 

class unetVggBNNAR1CLFRes(nn.Module):
    def __init__(
        self,
        input_channels,
        output_channels,
        ...
        super(unetVggBNNAR1CLFRes, self).__init__()
