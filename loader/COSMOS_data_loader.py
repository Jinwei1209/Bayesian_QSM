...
class COSMOS_data_loader(data.Dataset):
    ...
    def load_all_volumes(self):
        patches_RDFs, patches_MASKs, patches_N_stds, patches_QSMs = [], [], [], []
        ...
        for i_case in self.casesRange:
            ...
            for i_dir in range(ndir):
                ...
                N_std_aug = augment_data(N_std_dir, voxel_size=voxel_size, flip='', thetas=[-15, 15])
                ...
                #     patches_N_std = extract_patches(N_std_aug[i_aug, ...], self.patchSize, self.extraction_step)
        ...
        del patches_QSMs
        # self.patches_N_stds = np.concatenate(patches_N_stds, axis=0)[:, np.newaxis, ...]
        del patches_N_stds
        # self.patches_weights = dataterm_mask(self.patches_N_stds, self.patches_MASKs, Normalize=False)
    ...
    def load_one_dir(self, test_dir):
        ...
        for i_case in self.casesRange:
            if self.flag_smv:
                ...
                RDF_dir = np.real(RDF_dir * Mask_dir).astype(np.float32)
